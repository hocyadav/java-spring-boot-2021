 ...
 * @create 04-03-2021
 */
public class PrintEvenOdd2Thread { //simple class with 2 methods for printing even and odd, in main test class we can create runnable obj using lambda
        ...
        synchronized (this) {
            while (counter < 10) {// traverse from 1 to N
                while (counter % 2 != 0) { // not even check - then go to wait (means other thread task if that will complete then that will awake me)
                    wait();// this thread will go in sleep mode, wait is applied on Object
                }
